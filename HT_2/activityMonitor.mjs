...
let logPeriod = 60 * 1000 // Once per minute program appends the output to the log file
let lastTimeLogged = Date.now() - logPeriod
function getMostCPUIntensiveProcess() {
    exec(command, (_error, stdout, _stderr) => {
        const line = stdout.trim().split(/\s+/);
        const { name, cpu, mem } = parser(line)
