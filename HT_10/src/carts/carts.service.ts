...
import { IOrdersRepository } from '@src/orders/orders.repository'
...
export class CartsService {
  ...
  private async createCart(userId: string): Promise<Cart> {
    const newCart = await this._cartsRepository.createCart(
      new Cart(uuid(), userId, false, []),
    )
    if (!newCart) {
      throw new BadRequestException('Cart was not created')
    }
